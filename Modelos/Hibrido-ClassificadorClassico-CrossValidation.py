...
RESUME_TRAINING = True                                                              # Alterar para True se quiser carregar um checkpoint salvo
RESUME_CHECKPOINT_PATH = ("outputs_v7/20260525-105407_direct_reupload_gpu_seed42_fulltrain_q8_cv5/fold_01/progress/checkpoint_latest.pt") # Ajustar com o caminho do checkpoint salvo
...
# --------------------------------------------------------
# Quando USE_CROSS_VALIDATION = True, o pipeline ignora o split treino/val/teste
# tradicional e roda K folds estratificados. Em cada fold:
#   - separa-se um pedaco do treino para validacao (early stopping/scheduler)
#   - treina-se um modelo do zero (pesos quanticos e classicos reinicializados)
#   - avalia-se no fold de teste (porcao 1/K nunca vista no treino daquele fold)
# Ao final agregam-se accuracy/f1 (media e desvio) e a matriz de confusao somada.
USE_CROSS_VALIDATION = True
CV_N_SPLITS = 5
CV_SHUFFLE = True
CV_VAL_FRACTION_OF_TRAIN = 0.10  # fracao do treino do fold reservada para validacao
CV_USE_TRAIN_SAMPLING = False    # se True, aplica TRAIN_SAMPLE_SIZE em cada fold de treino
...
# =========================================================
# 11.b) CROSS VALIDATION (Stratified K-Fold)
# =========================================================
def run_cross_validation(
    ...
    """Stratified K-Fold com val interno para early stopping em cada fold.

    Em cada fold:
      - Re-encapsula features com Preprocessor novo (fit so no treino do fold).
      - Re-instancia o modelo (TorchConnector + head) zerando pesos.
      - Treina com early stopping pelo val_f1 do val interno.
      - Avalia no fold de teste e acumula y_true/y_pred globais.
    """
    label_encoder = LabelEncoder()
    y_all = label_encoder.fit_transform(df["class"].to_numpy())
    idx_all = np.arange(len(df))
    ...
    # Se for para retomar, tenta localizar o ultimo CV dir existente em `output_root`
    cv_dir = None
    start_fold = 1
    if RESUME_TRAINING:
        out_root = Path(output_root)
        if out_root.exists():
            pattern = f"*cv{CV_N_SPLITS}"
            candidates = sorted(
                [p for p in out_root.iterdir() if p.is_dir() and p.name.endswith(f"_cv{CV_N_SPLITS}")],
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )

            # Prioriza o mais recente que contenha checkpoints em algum fold.
            selected = None
            for cand in candidates:
                has_checkpoint = False
                for p in cand.iterdir():
                    if p.is_dir():
                        m = re.match(r"fold_(\d+)", p.name)
                        if m:
                            prog = p / PROGRESS_DIR_NAME / "checkpoint_latest.pt"
                            if prog.exists():
                                has_checkpoint = True
                                break
                if has_checkpoint:
            ...
            # Se nenhum candidato contiver checkpoints, usa o mais recente mesmo assim.
            if selected is None and candidates:
                selected = candidates[0]

            if selected is not None:
                cv_dir = selected
                print(f"[RESUME] Encontrado CV dir existente (priorizando checkpoints): {cv_dir}")

                # procura folds com checkpoint existente
                fold_idxs = []
                for p in cv_dir.iterdir():
                    if p.is_dir():
                        ...
                        if m:
                            prog = p / PROGRESS_DIR_NAME / "checkpoint_latest.pt"
                            if prog.exists():
                                fold_idxs.append(int(m.group(1)))

                if fold_idxs:
                    # recomeça no fold onde havia checkpoint (o máximo detectado)
                    start_fold = max(fold_idxs)
                    print(f"[RESUME] Irei recomeçar no fold {start_fold:02d} (checkpoint detectado)")

    if cv_dir is None:
        cv_experiment_id = generate_experiment_id() + f"_cv{CV_N_SPLITS}"
        cv_dir = Path(output_root) / cv_experiment_id
        ensure_dir(cv_dir)
    else:
        cv_experiment_id = cv_dir.name

    # Modo de teste: se a variável de ambiente RESUME_PRINT_ONLY estiver definida,
    # localiza o checkpoint do fold detectado (start_fold) e imprime o campo 'epoch'
    # para verificar de onde o retomar ocorreria, então sai.
    if os.environ.get("RESUME_PRINT_ONLY"):
        probe_fold_dir = cv_dir / f"fold_{start_fold:02d}"
        probe_checkpoint = probe_fold_dir / PROGRESS_DIR_NAME / "checkpoint_latest.pt"
        if probe_checkpoint.exists():
            try:
                chk = torch.load(probe_checkpoint, map_location="cpu")
                epoch_saved = chk.get("epoch", None)
                print(f"[RESUME_PRINT] checkpoint encontrado: {probe_checkpoint}")
                print(f"[RESUME_PRINT] checkpoint['epoch'] = {epoch_saved}")
            except Exception as exc:
                print(f"[RESUME_PRINT] falha ao carregar checkpoint: {exc}")
        else:
            print(f"[RESUME_PRINT] nenhum checkpoint em: {probe_checkpoint}")
        sys.exit(0)
    ...
    print(f"\n===== STRATIFIED {CV_N_SPLITS}-FOLD CROSS VALIDATION =====")

    folds = list(skf.split(idx_all, y_all))
    total_folds = len(folds)
    for i in range(start_fold - 1, total_folds):
        fold_idx = i + 1
        train_dev_idx, test_idx = folds[i]
        fold_start = time.perf_counter()
        print(f"\n----- Fold {fold_idx}/{CV_N_SPLITS} -----")

        y_train_dev = y_all[train_dev_idx]

        # Split interno treino/val (estratificado) para early stopping.
        idx_train, idx_val, _, _ = train_test_split(
            train_dev_idx,
            y_train_dev,
        ...
        if CV_USE_TRAIN_SAMPLING and TRAIN_SAMPLE_SIZE < len(df_train):
        ...
        model, _qnn, _estimator, _actual_aer, torch_device = build_model(
            qc=qc,
            data_vector=data_vector,
            ansatz=ansatz,
            observables=observables,
        ...
        fold_progress_dir = cv_dir / f"fold_{fold_idx:02d}" / PROGRESS_DIR_NAME
        ensure_dir(fold_progress_dir)

        # Se RESUME_TRAINING=True, encaminha o checkpoint do progresso do fold
        # (se existir, o próprio `train_model` verifica a existência antes de carregar).
        resume_path = (fold_progress_dir / "checkpoint_latest.pt") if RESUME_TRAINING else None
        ...
        best_state, best_val_f1, best_epoch, history = train_model(
            ...
            n_classes=n_classes,
            device=torch_device,
            progress_dir=fold_progress_dir,
            resume_checkpoint_path=resume_path,
        )

        test_acc, test_f1, y_true_fold, y_pred_fold = evaluate_model(
            model=model,
        ...
        print(
            f"Fold {fold_idx} | best_epoch={best_epoch} | best_val_f1={best_val_f1:.4f} | "
            f"test_acc={test_acc:.4f} | test_f1={test_f1:.4f} | tempo={fold_time/60:.2f} min"
        )

        # Relatorio por fold.
        fold_dir = cv_dir / f"fold_{fold_idx:02d}"
        ensure_dir(fold_dir)
        ...
        save_json(report_dict, fold_dir / "classification_report.json")
        save_text(report_txt, fold_dir / "classification_report.txt")
        cm_fold = confusion_matrix(y_true_fold, y_pred_fold, labels=list(range(n_classes)))
        np.save(fold_dir / "confusion_matrix.npy", cm_fold)
        plot_and_save_confusion_matrix(cm_fold, class_names, fold_dir / "confusion_matrix.png")
    ...
    metrics_df.to_csv(cv_dir / "fold_metrics.csv", index=False)

    accs = metrics_df["test_acc"].to_numpy()
    f1s = metrics_df["test_f1"].to_numpy()
    cv_summary = {
        "experiment_id": cv_experiment_id,
        ...
        "test_f1_min": float(f1s.min()),
        "test_f1_max": float(f1s.max()),
        "per_fold": fold_metrics,
    }
    save_json(cv_summary, cv_dir / "cv_summary.json")

    # Confusao agregada (concatenacao das predicoes de todos os folds).
    cm_total = confusion_matrix(all_y_true, all_y_pred, labels=list(range(n_classes)))
    np.save(cv_dir / "confusion_matrix_total.npy", cm_total)
    plot_and_save_confusion_matrix(cm_total, class_names, cv_dir / "confusion_matrix_total.png")

    report_total = classification_report(
        all_y_true, all_y_pred,
        target_names=class_names, output_dict=True, zero_division=0,
    )
    report_total_txt = classification_report(
        all_y_true, all_y_pred,
        target_names=class_names, digits=4, zero_division=0,
    )
    save_json(report_total, cv_dir / "classification_report_total.json")
    save_text(report_total_txt, cv_dir / "classification_report_total.txt")

    full_report_lines = [
        "=== CROSS VALIDATION FULL REPORT ===",
        f"experiment_id: {cv_experiment_id}",
        f"n_splits: {cv_summary['n_splits']}",
        f"test_acc_mean: {cv_summary['test_acc_mean']:.4f}",
        f"test_acc_std: {cv_summary['test_acc_std']:.4f}",
        f"test_f1_mean: {cv_summary['test_f1_mean']:.4f}",
        f"test_f1_std: {cv_summary['test_f1_std']:.4f}",
        f"test_acc_min: {cv_summary['test_acc_min']:.4f}",
        f"test_acc_max: {cv_summary['test_acc_max']:.4f}",
        f"test_f1_min: {cv_summary['test_f1_min']:.4f}",
        f"test_f1_max: {cv_summary['test_f1_max']:.4f}",
        "",
        "--- Fold metrics ---",
    ]
        ...
        full_report_lines.append(
            " | ".join(
                [
                    ...
                    f"best_val_f1={fold['best_val_f1']:.4f}",
                    f"test_acc={fold['test_acc']:.4f}",
                    f"test_f1={fold['test_f1']:.4f}",
                    f"train_size={fold['train_size']}",
                    f"val_size={fold['val_size']}",
                    f"test_size={fold['test_size']}",
                    f"fold_time_sec={fold['fold_time_sec']:.1f}",
                ]
            )
        )
    full_report_lines.extend([
        ...
        *report_total_txt.splitlines(),
        "",
        f"Fold reports saved in: {cv_dir}",
        "",
    ])
    save_text("\n".join(full_report_lines), cv_dir / "cv_full_report.txt")

    print("\n===== RESULTADO CROSS VALIDATION =====")
    print(f"Test Accuracy: {cv_summary['test_acc_mean']:.4f} +/- {cv_summary['test_acc_std']:.4f}")
    print(f"Test Macro F1: {cv_summary['test_f1_mean']:.4f} +/- {cv_summary['test_f1_std']:.4f}")
    print(f"Min/Max Acc:   {cv_summary['test_acc_min']:.4f} / {cv_summary['test_acc_max']:.4f}")
    print(f"Min/Max F1:    {cv_summary['test_f1_min']:.4f} / {cv_summary['test_f1_max']:.4f}")
    print(f"Relatorio CV salvo em: {cv_dir}")
    print("======================================\n")

    return cv_summary, cv_dir
...
def main():
    ...
    # Quando rodamos K-Fold, o split tradicional treino/val/teste e' substituido
    # pela rotina run_cross_validation. Ainda precisamos de class_names e N_CLASSES.
    if USE_CROSS_VALIDATION:
        class_names = sorted(df["class"].astype(str).unique().tolist())
        ...
        print(f"\nUsando {CV_N_SPLITS}-Fold Cross Validation. Total amostras: {len(df)}")

        cv_summary, cv_dir = run_cross_validation(
            df=df,
        ...
        cv_summary["total_runtime_sec"] = float(total_time)
        cv_summary["total_runtime_min"] = float(total_time / 60)
        cv_summary["total_runtime_hours"] = float(total_time / 3600)
        save_json(cv_summary, cv_dir / "cv_summary.json")
